...
{
	Logger log(NUM_PLAYER,test_case);// the logger. It will be past in where it is need
...
void Initialize(Logger& log)							// Initialize before playing each time
{
	current_cards=52;
	divided_cards=0;
	current_card="";
...
//	cout<<"Cards have been divided for "<<NUM_PLAYER << " players"<<endl;
	...
	log.CardLog(players);//log card each player have
